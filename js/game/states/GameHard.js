...
WizardClash.GameHard.prototype = {
    create: function() {
...
//--------------------------------------------------------------------------------------------------------------------------
    ...
    createEnemy: function() {
        ...
        this.enemies.forEach(function(enemy) {
            game.physics.arcade.moveToObject(enemy, wizard.body, 800); //enemies will follow the player at all times
        });
    ...
    enemyHit: function(player, enemy) {
        if (!wizard.invincible) { //if the player's invincibility frames are not currently playing the moment the player collides with an enemy
            ...
            if (usingShield === true) {
                this.health = this.health - 10; //if the player happened to be inside a shield at the time of collision, damage to its health will be reduced
            } else {
                this.health = this.health - 20;
            }
            this.healthText.text = 'Health: ' + this.health;
            game.time.events.add(1000, this.toggleInvincible, this); //if the player is hit by an enemy, it will be invincible for two seconds before its normal properties are retained
            game.time.events.add(1000, this.restoreTint, this); //this function restores the player's tint at the same time that the player loses its invincibility state
